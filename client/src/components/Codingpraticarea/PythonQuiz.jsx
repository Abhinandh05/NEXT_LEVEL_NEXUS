...
const PythonQuiz = () => {
  ...
  const [answers, setAnswers] = useState(pythonQuestions.map(() => ""));
  const [feedback, setFeedback] = useState("");
  const [attempts, setAttempts] = useState(pythonQuestions.map(() => null));
  ...
  const handleTimeOut = () => {
    ...
    navigateQuestion(
      Math.min(currentQuestionIndex + 1, pythonQuestions.length - 1)
    );
  };
      ...
      <header className="bg-gray-800 py-4 px-6 flex justify-between items-center shadow-md">
        ...
        <div className="flex items-center space-x-6">
          <span className="text-gray-400 text-sm">
            Question {currentQuestionIndex + 1}/{pythonQuestions.length}
          </span>
          <div
          ...
          <ul>
            {pythonQuestions.map((_, index) => (
              ...
              >
                <span>Q{index + 1}</span>
                <span
                  className={`w-3 h-3 rounded-full ${getAttemptStatus(index)}`}
                ></span>
              </li>
            ))}
          </ul>
