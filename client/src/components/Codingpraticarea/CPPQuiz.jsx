...
const CPPQuiz = () => {
  ...
  const [answers, setAnswers] = useState(CPPQuestions.map(() => ""));
  const [feedback, setFeedback] = useState("");
  const [attempts, setAttempts] = useState(CPPQuestions.map(() => null));
  ...
  const handleTimeOut = () => {
    ...
    navigateQuestion(Math.min(currentQuestionIndex + 1, CPPQuestions.length - 1));
  };
      ...
      <header className="bg-gray-800 py-4 px-6 flex justify-between items-center shadow-md">
        ...
        <div className="flex items-center space-x-6">
          <span className="text-gray-400 text-sm">
            Question {currentQuestionIndex + 1}/{CPPQuestions.length}
          </span>
          <div
        ...
        <main className="w-3/5 flex flex-col p-6 space-y-6">
          <div className="bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col space-y-4 transition-transform transform hover:scale-105 duration-300">
            <h2 className="text-xl font-bold text-yellow-400">
              {CPPQuestions[currentQuestionIndex].question}
            </h2>
            <textarea
            ...
            <div className="flex space-x-4">
              ...
              <button
                onClick={() =>
                  navigateQuestion(
                    Math.min(currentQuestionIndex + 1, CPPQuestions.length - 1)
                  )
                }
