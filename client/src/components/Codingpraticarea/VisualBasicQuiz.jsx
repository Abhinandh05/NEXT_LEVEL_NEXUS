...
import VisualBasicQuestions from "../../data/VisualBasicQuestions";

const VisualBasicQuiz = () => {
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [answers, setAnswers] = useState(VisualBasicQuestions.map(() => ""));
  const [feedback, setFeedback] = useState("");
  const [attempts, setAttempts] = useState(VisualBasicQuestions.map(() => null));
  ...
  const handleTimeOut = () => {
    ...
    navigateQuestion(Math.min(currentQuestionIndex + 1, VisualBasicQuestions.length - 1));
  };

  const handleAnswerSubmit = () => {
    const currentQuestion = VisualBasicQuestions[currentQuestionIndex];
    const isCorrect =
    ...
    <div className="h-screen flex flex-col bg-gray-900 text-white font-sans">
      ...
      <header className="bg-gray-800 py-4 px-6 flex justify-between items-center shadow-md">
        ...
        <div className="flex items-center space-x-6">
          <span className="text-gray-400 text-sm">
            Question {currentQuestionIndex + 1}/{VisualBasicQuestions.length}
          </span>
          <div
      ...
      <div className="flex flex-grow overflow-hidden">
        ...
        <aside className="w-1/5 bg-gray-800 overflow-y-auto border-r border-gray-700 shadow-lg">
          ...
          <ul>
            {VisualBasicQuestions.map((_, index) => (
              <li
                key={index}
                onClick={() => navigateQuestion(index)}
                className={`cursor-pointer px-4 py-2 flex items-center justify-between hover:bg-gray-700 transition-colors duration-300 ${currentQuestionIndex === index ? "bg-blue-600" : ""}`}
              >
                <span>Q{index + 1}</span>
        ...
        <main className="w-3/5 flex flex-col p-6 space-y-6">
          <div className="bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col space-y-4 transition-transform transform hover:scale-105 duration-300">
            <h2 className="text-xl font-bold text-yellow-400">
              {VisualBasicQuestions[currentQuestionIndex].question}
            </h2>
            <textarea
            ...
            <div className="flex space-x-4">
              ...
              <button
                onClick={() => navigateQuestion(Math.min(currentQuestionIndex + 1, VisualBasicQuestions.length - 1))}
                className="bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-600 transform transition-transform hover:scale-105"
              >
                Next
              </button>
              <button
                onClick={() => navigateQuestion(Math.max(currentQuestionIndex - 1, 0))}
                className="bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-600 transform transition-transform hover:scale-105"
              >
                Previous
              </button>
          ...
          {feedback && (
            <div
              className={`p-4 rounded text-center transition-all duration-500 ${
                feedback.includes("✅")
...
export default VisualBasicQuiz;
